...
public class Semant {
	...
	// Wherever a var entry is constructed you must first, allocate the var,
	// before constructing an entry for it with the result

	// When constructing an entry for a function you must first, construct a new
	// level for it with
	// information as to which parameters to the function escape, before
	// constructing the function
	// entry with the new level
	// The function body needs to be translated in this new function level

	// VarEntry and LoopVarEntry will both need an additional Access parameter
	// added to the constructor

	public void transProg(Absyn.Exp exp) {
		// Find all vars that escape
		FindEscape.FindEscape findEsc = new FindEscape.FindEscape(exp);
	...
	private Exp checkComparable(ExpTy et, int pos) {
		Type a = et.ty.actual();
		if (!(a instanceof Types.INT || a instanceof Types.STRING
				|| a instanceof Types.NIL || a instanceof Types.RECORD || a instanceof Types.ARRAY))
	...
	ExpTy transExp(Absyn.ForExp e) {
		...
		env.venv.beginScope();
		Translate.Access newAccess = level.allocLocal(e.var.escape);
		e.var.entry = new LoopVarEntry(INT, newAccess);
	...
	Exp transDec(Absyn.VarDec d) {
		// Wherever a var entry is constructed you must first, allocate the var,
		// before constructing an entry for it with the result
		Translate.Access newAccess = level.allocLocal(d.escape);
		...
		d.entry = new VarEntry(type, newAccess);
	...
	private Types.RECORD transTypeFields(Hashtable hash, Absyn.FieldList f) {
		...
		if (hash.put(f.name, f.name) != null)
			error(f.pos, "function parameter/record field redeclared: "
					+ f.name);
		return new Types.RECORD(f.name, name, transTypeFields(hash, f.tail));
	}
	
	private void putTypeFields(Types.RECORD f, Translate.AccessList al) {
		if (f == null)
			return;
		env.venv.put(f.fieldName, new VarEntry(f.fieldType, al.head));
		putTypeFields(f.tail, al.tail);
	}
